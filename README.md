...
> **计算单元 v2（Mac 远程跑批）已于 2026-08-18 退役**（ADR-17）：本地 `discovery run`/daemon
> 已覆盖其使命；回测内核指纹子系统（ENGINE_FILES/engine_hash）迁 `discovery/fingerprint.py`
> 单源存续，老 trial 可比性语义不变。
...
| **参数发现引擎** | `python -m discovery` / 02:00 cron | Plan 1-4 闭环：L0 快照 → L1 OOS 裁判 → L2 采样 → L3 帕累托/TPE → L4 daemon → L5 publish；内核指纹 `discovery/fingerprint.py`（compute_unit 退役遗产，ADR-17） |
