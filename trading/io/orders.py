...
契约透传（gateway_service.submit_order）：
...
Why 不在本层做挡板/风控判定：
    check_order 10 关风控已在 gateway_service.submit_order 内部调 trading.compute.risk
    （functional core），io/ 只是搬运到 gateway_service 这条路径，不重复判定也不下推。
"""
...
async def submit_order(order: Any, *, dry_run: bool, confirm: bool = True) -> dict:
    """下单 I/O 包装（透传 gateway_service.submit_order）。
