# -*- coding: utf-8 -*-
"""trading.compute.reconcile — 持仓对账分类纯函数（functional core）。

物理定位（Layer2 阶段2 · spec §3.5/§4）：
    reconcile 是【纯函数】——无副作用、无 I/O、可独立单测。把「本地系统理论持仓」
    与「券商真实持仓」比对，暴露敞口偏差——实盘风控核心：drifted（数量漂移）、
    only_local（疑似未成交/丢单）、only_broker（疑似外部成交/手动单）三类差异各自
    指向不同的风险场景。仅依赖标准库 dataclasses/typing，零外部依赖。
