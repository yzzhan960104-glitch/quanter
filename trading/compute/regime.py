...
当日缓存：宽度计算要读 455MB 主湖——读湖与当日缓存归 trading.data_ctx.
load_regime_frames（compute 纯度守卫：零 I/O），eod 前置与 pre_open ④ 段
共享一次读湖；盘中新数据不改当日判定（regime 是环境闸非交易信号）。
"""
...
def classify(index_df, daily_df, asof=None) -> RegimeState:
    """三态 regime 判定（纯计算，df 注入——读湖装配见 data_ctx.load_regime_frames）。

    Args:
        index_df: 指数日线（index=[date, symbol]，close 列——HS300 腿）。
        daily_df: 全市场日线（index=[date, symbol]，close 列——宽度腿）。
        asof: 显式判定基准日（回放/测试用）；None = 湖数据最新日（生产路径）。
    计算任何异常 → UNKNOWN（fail-closed 语义由调用方执行停手，本模块不放行）。
    """
