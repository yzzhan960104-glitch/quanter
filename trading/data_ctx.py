...
# ============================================================================
# A1（DG-G4 · 2026-08-14）：regime 闸数据装配（IO 层——compute 纯度守卫要求
# 读湖不进 trading/compute，本模块是数据装配的正当归属）
# ============================================================================
def load_regime_frames():
    """读 regime 判定所需两湖（index_daily / a_shares_daily），当日进程内缓存。

    物理意图：regime.classify 是 compute 纯函数（df 注入），读湖 IO 归本装配层
    （tests/test_compute_purity 守卫：compute 零外部 I/O）。当日缓存——455MB
    主湖一天读一次（eod 前置 + pre_open ④ 段共享），盘中新数据不改当日判定
    （regime 是环境闸非交易信号）。
    读湖异常向上抛（调用方 engine._regime_gate 的 fail-closed 兜底捕获→停手）。
    """
    import datetime as _dt
    import pandas as pd

    today = _dt.date.today().isoformat()
    cached = _REGIME_FRAMES_CACHE.get(today)
    if cached is not None:
        return cached
    idx = pd.read_parquet("data_lake/index_daily.parquet")
    daily = pd.read_parquet("data_lake/a_shares_daily.parquet")
    _REGIME_FRAMES_CACHE[today] = (idx, daily)
    # 单一 return 源：缓存命中与首次读湖返回同一 tuple 对象（调用方可 is 判缓存）
    return _REGIME_FRAMES_CACHE[today]
