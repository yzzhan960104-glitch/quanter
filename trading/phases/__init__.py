...
测试 patch 路径约定（brief 关键上下文 §3）：
    本包内函数对原 engine 模块级符号（``_submit`` / ``_state_store`` / ``get_gateway`` /
    ``_cancel_all_open_orders`` 等）统一经函数内 lazy ``import trading.engine as _eng_mod``
    反查——规避循环 import（engine 顶部 re-export 本包）+ 保既有 ``patch("trading.engine._xxx")``
    / ``monkeypatch.setattr(engine, "_xxx", ...)`` 测试断言全命中（详见各子模块 docstring）。
"""
