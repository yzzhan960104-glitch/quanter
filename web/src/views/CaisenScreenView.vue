...
const scanForm = ref<ScanRequestBody>({
  date: today,
...
function initChart() {
  ...
  const bg = cs.getPropertyValue('--qt-bg-card').trim() || '#1e222d'
  const text = cs.getPropertyValue('--qt-text-primary').trim() || '#d1d4dc'
  const grid = cs.getPropertyValue('--qt-border').trim() || '#2b3139'
  const up = cs.getPropertyValue('--qt-up').trim() || '#ef5350'
  const down = cs.getPropertyValue('--qt-down').trim() || '#26a69a'
...
async function onScan() {
  scanning.value = true
  try {
    const result = await scan(scanForm.value)
