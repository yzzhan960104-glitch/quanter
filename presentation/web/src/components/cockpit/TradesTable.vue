<!--
  TradesTable —— 交易流水卡（一期观测运营层 · Task 9；09-01 全字段升级；
  09-03 P5.1 Tab 化：成交|委托同卡，OrdersPanel 承载委托半场）。

  物理意图：驾驶舱「交易流水」卡——成交流水全字段作战记录：时间/方向/标的+
  公司名/价格/数量/金额/委托号；「当日委托」Tab=柜台 orders 全状态全景
  （含拒单拒因）。数据=GET /api/v1/gm/trades（在线）或静态快照
  （成交=gm_trades_history 跨日史、委托=gm_orders）；公司名经 ohlcv 快照
  富化（缺快照 '—' 不猜）。
-->
<template>
  <el-card shadow="never">
    ...
    <el-tabs v-model="tab" class="flow-tabs">
      <el-tab-pane label="成交流水" name="trades">
      ...
      <el-tab-pane label="当日委托" name="orders" lazy>
        <OrdersPanel />
      </el-tab-pane>
    </el-tabs>
  </el-card>
...
/* Tab 化（P5.1）：成交|委托同卡切换——EP tabs 默认下边距吞高度，收紧对齐表格 */
.flow-tabs :deep(.el-tabs__header) { margin-bottom: 8px; }
