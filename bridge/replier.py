...
async def reply(
    ...
    for i, chunk in enumerate(chunks):
        ...
        for attempt in range(3):
            try:
                # reply_text：dingtalk-stream ChatbotHandler 自带的 @回复方法（同步）。
                # Why to_thread：reply_text 是同步方法（内部 requests.post 阻塞），
                # 直接 await 会抛 TypeError（None 非 awaitable）且阻塞事件循环；
                # 故用 asyncio.to_thread 扔线程池执行。
                # 参数顺序按真实签名 (text, incoming_message)：payload 文本在前、
                # incoming_msg 消息对象在后（之前两参数顺序反了）。
                await asyncio.to_thread(handler.reply_text, payload, incoming_msg)
                break
