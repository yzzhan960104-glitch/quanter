...
def test_snapshot_reads_all_six_tables_plus_plan(isolated_state):
    """snapshot(T+1) → 读 trade_event/order/fill/position/account/account_daily + plan 计数。"""
    ...
    # N3 收口：原手写「save_plan_legacy JSON 镜像 + 账户 upsert + 逐单 DB SIGNAL」改调
    # 共享 seed_plan。参数逐字复刻老行为：account_id="e2e_long_acc" 显式覆盖（老硬编码
    # 口径，不随 env 解析漂移）、confirmed=False（老代码只写 SIGNAL 不写 CONFIRMED——
    # 本断言只读 plan_orders，确认态不在验证面）、json_mirror=True（保留 JSON 展示镜像；
    # plan_orders 实际经 load_plan 读 DB SIGNAL——JSON 对 load_plan 恒不可见）。
    _orders_e2e = [
    ...
    seed_plan("2026-07-02", _orders_e2e, confirmed=False, json_mirror=True,
              account_id="e2e_long_acc")
