...
闸后的 6 种边界：
  ...
  1. mode=live + 影子不足（activated_at < 5天）         → sys.exit(2)（D3 fail-closed）
  2. mode=live + 所有影子充足（≥5天）                    → 放行返 True
  3. mode=live + activated_at=None（缺失）               → 保守拒绝（D9 宁可误杀）
  4. mode=live + resolve_active 抛异常                    → fail-closed exit(2)（不降级继续）
  5. mode=dry_run                                          → 跳过（不查 resolve_active）
  6. mode=live + resolve_active 返空列表                   → 放行（D8 合法清场）
