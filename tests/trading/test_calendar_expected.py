...
T9（2026-08-15）：expected_latest_trade_day 物理实现已自 trading/calendar.py 下沉至
data/calendar.py（纯交易日历域函数归 data 层，data→trading 边清零）。本测试仍经
trading.calendar 的 re-export 导入（固化兼容契约：engine.py:724 等消费者零改动），
但 patch 锚须落**定义模块** data.calendar——patch 替换的是指定模块的属性绑定，
函数体只在定义模块的全局命名空间解析交易日判定（_is_trading_day），patch
trading.calendar.is_trading_day 已影响不到下沉后的实现。
"""
...
def test_after_close_on_trading_day_returns_today():
    ...
    with patch("data.calendar._is_trading_day", return_value=True):
...
def test_before_close_returns_previous_trade_day():
    ...
    with patch("data.calendar._is_trading_day", side_effect=lambda d: d == "2026-07-22"):
...
def test_weekend_rolls_back_to_friday():
    ...
    with patch("data.calendar._is_trading_day", side_effect=lambda d: d == "2026-07-24"):
...
def test_non_trading_day_after_close_rolls_back():
    ...
    with patch("data.calendar._is_trading_day", return_value=False):
