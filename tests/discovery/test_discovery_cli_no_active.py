...
物理意图（2026-08-05 双轨治理收口）：
    ...
    ``experiment.db ACTIVE``（resolve_active 单一真相源，实盘 _eod 同源）读。
    设计语义：**无 ACTIVE 实验 → 拒绝运行（exit 2）**——没有当前生效冠军，
    oos/verify 无从跑「当前冠军」去偏，应立即可见地失败而非静默走默认/legacy。

    本 fast 单测覆盖该早返路径：monkeypatch ``resolve_active`` 返 ``[]``（无 ACTIVE），
    ...
    **不跑真 oos/verify**（真跑需 freeze 全 data_lake + 全历史 scan_symbol ~3-4min，
    ...
    早返发生在 args 字段访问之前（``active = resolve_active()`` → ``if not active: sys.exit(2)``
...
def test_cmd_oos_no_active_exits_2(monkeypatch):
    ...
    # monkeypatch resolve_active 返空 list（无 ACTIVE）
    monkeypatch.setattr(cli, "resolve_active", lambda: [])
