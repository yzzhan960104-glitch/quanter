...
from ops.run_data_check import run_check, _resync_key


def test_checkpoint1_t1_pass_no_alert():
    """检查点①：T-1 齐全 → 返 OK，不熔断。"""
    from data.freshness import FreshnessResult
    with patch("ops.run_data_check.check_freshness",
...
def test_checkpoint2_t_fail_triggers_resync_until_pass():
    ...
    with patch("ops.run_data_check.check_freshness", side_effect=[fail, ok]), \
         patch("ops.run_data_check.sync_daily_incremental", sync), \
         patch("ops.run_data_check._now", side_effect=["18:30", "18:45"]):
...
def test_checkpoint2_t_fail_after_deadline_melts():
    ...
    from data.freshness import FreshnessResult
    fail = FreshnessResult("daily", False, "2026-07-22", "2026-07-23", "陈旧")
    with patch("ops.run_data_check.check_freshness", return_value=fail), \
         patch("ops.run_data_check.sync_daily_incremental",
               side_effect=RuntimeError("积分不足")), \
         patch("ops.run_data_check._now", return_value="20:30"):  # 已超 20:00
        r = run_check("t2", keys=("daily",), deadline_hour=20)
...
def test_checkpoint2_multi_round_resync_then_pass():
    ...
    ok = FreshnessResult("daily", True, "2026-07-23", "2026-07-23", "PASS")
    sync = MagicMock(return_value="OK 最新日 2026-07-23")
    with patch("ops.run_data_check.check_freshness", side_effect=[fail, fail, ok]), \
         patch("ops.run_data_check.sync_daily_incremental", sync), \
         patch("ops.run_data_check.time.sleep", side_effect=lambda _: None) as mock_sleep, \
         patch("ops.run_data_check._now",
...
def test_resync_key_daily_routes_to_sync_daily_incremental():
    ...
    with patch("ops.run_data_check.sync_daily_incremental", sdi), \
         patch("ops.run_data_check.sync_one_key", sok):
...
def test_resync_key_non_daily_falls_through_to_sync_one_key():
    ...
    with patch("ops.run_data_check.sync_one_key", sok), \
         patch("ops.run_data_check.sync_daily_incremental", sdi):
