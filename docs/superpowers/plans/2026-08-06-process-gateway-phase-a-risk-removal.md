...
> D1=保留断线保护/幂等/审计；D2=保留 session 关，窗口 09:15–15:00 **连续**（午休不拦，
> 2026-08-06 用户选项 1 修订；A1 已修 09:15 起点）；
> D3=删除白名单（前端同步放开）；D4=影子闸降级 WARN 不阻断；D5=删日内 -3% 自动熔断、
> 保留 emergency_halt+告警；D6=验收=全量 pytest + audit_ssot + 下一交易日 09:22 实测；
> D7=删除实现先过 dry_run 回归（不切生产 .env 到 dry_run）。
