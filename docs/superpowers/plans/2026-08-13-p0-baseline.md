...
def main():
    ...
    for fold, years in folds.items():
        ...
        print(f"  折 {fold:>8}：min 创板科创 = {cov:>5}  {'✅' if ok else '⚠️不足'}", flush=True)
    verdict = "✅ 支撑 P5 分折定义" if feasible else "⚠️ 部分折覆盖不足 → Task 5 调整 P5 折定义/阈值"
...
### P0-2 湖深
- 逐年创板科创覆盖：2020=__  2021=__  2022=__  2023=__  2024=__  2025=__  2026=__
- 退市/休眠创板科创 = __ 只
- 科创开板约束对 2020-21 折影响：__
- P5 分折可行性 verdict = (✅/⚠️)
