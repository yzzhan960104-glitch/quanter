...
### P0-1 热点确认
- 单标的预期占比 = 47.4% ；30 标的预期占比 = 47.9% ；gate(min) = 47.4% → WARN（偏离 spec 假设，需 Task 5 复核 P1 优先级）
- top-5 热点函数（tottime 降序）：
  1. `{method 'reduce' of 'numpy.ufunc' objects}`
  2. `search_neckline` (strategies/neckline/method_v0.py:111)
  3. `local_minima` (strategies/neckline/method_v0.py:84)
  4. `{method 'max' of 'numpy.ndarray' objects}`
  5. `__finalize__` (pandas/core/generic.py:6258)
- 结论：P1 向量化目标函数集 = search_neckline + local_minima + numpy ufuncs（reduce/max/min）
  **注记**：预期热点池占比 47.4% < 70% 验收门，说明大量时间消耗在 numpy 内部函数（ufunc reduce/max/min）和 pandas 操作（__finalize__/__getitem__）而非预期池的 Python 循环。P1 向量化仍应针对颈部算法（search_neckline/local_minima），但需考虑 numpy 优化（向量化替代逐日 reduce）或 pandas 调用优化（减少 __finalize__ 开销）。Task 5 据此复核 P1 优先级。
