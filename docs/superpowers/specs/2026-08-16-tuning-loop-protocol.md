...
豁免条款（ADR-15 §口径声明同源）：autopromote 的 **G4（wf 跨年）/G5（邻域高原）/G6（DSR）** 属稳健性统计闸，度量「参数是否孤峰/是否多重比较虚高/是否跨年稳定」，非收益水平指标——豁免本条（scan 口径下分辨率更高）；**收益的放行判据专属 G2/G3（replay 口径）**，两族闸不可互相替代。
