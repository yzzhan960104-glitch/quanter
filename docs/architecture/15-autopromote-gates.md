...
- **R3（2026-08-23）**：G2/G3 评估口径切换——outer 段指标从 raw replay 改为**人工风控模拟线**（`discovery/manual_risk_sim`，池子等权 20d 滚动回撤 −15% 触发/−10% 解除的滞回日历，基线与候选同一日历）。依据：用户裁决「震荡/趋势市况尽量最大盈利，宏观深度回撤由人工风控保证」——策略对可交易期收益负责；R2 实证 raw outer 被 2026-07 池子 −18.5% 回撤主导（结构性年段切换），把可交易期更好的候选误判为劣。阈值经选段（2021-2024）校准 + 验证段两项硬判据（07 月覆盖 ✓/03 月不误触 ✓，`docs/research/2026-08-23-r3-manual-risk-calibration.md`）。门槛数值（G1/G2/G3 阈值）全部未动。ADR-16 红线重申：模拟线是回测评估假设，`trading/` 全树禁 import `discovery`（层契约铁律 7 机械化）。
