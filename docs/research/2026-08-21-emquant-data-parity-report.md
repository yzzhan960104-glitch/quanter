...
### 实测结果（2026-08-21 08:40 晨间补跑，token 从终端 UI 复制后）

**列级（compare_data.py，容差 1e-6）**：参与 300 | 一致 90 | 不一致 201 | 窗口不足 9；
一致率 **30.93%**；最大相对偏差 2.85e-01（300806.SZ @ 2025-12-05 low）。

**关键解读：不一致的主体是噪声不是分歧**——201 只中绝大多数 max_rel 落在
1e-6~1e-5、四列比率中位 ≈0.999996~1.0000013（两源复权因子浮点精度差，对信号
无实际传导）；少数系统性偏移（300042.SZ 全列 0.999988 ≈ 因子精度）与极少数
单日大偏差（300806.SZ 28.5%，疑数据源单点修复不同步）才是真分歧。

**信号级（parity_signals.py，容差 1e-4，锚 2026-08-20）**：可比 291 |
双侧有信号 0 | 双侧无信号 291 | **presence 分叉 0 | 字段超差 0** →
**信号级一致率 100.00%**（当日无突破信号属正常稀疏；字段级证据随 W2 逐日累积）。

**W2 分母裁定**：排除池维持保守口径（210 条列级不一致全豁免，比对域≈90 只
+ 逐日新增信号时按 presence 同侧性即时验证）；噪声级不一致**不阻塞**试点——
信号级对拍才是设计 §7 的验收口径，列级排除池只是防真分歧的保险带。

---

## 五、遗留与移交

- [x] token 生成 + 三步补跑（2026-08-21 晨完成；信号级 100% 见第四节）
- [x] `gmterm-serv` 进程名注记并入 README 的终端自检命令（1.1 的 findstr 陷阱）
- [x] 补跑后回填第四节实测三数；W2 双轨对照分母按排除池收敛（保守口径维持）
- [ ] W2 期间逐日复核：信号级 presence 同侧性 + `known_divergence` 标记行剔除口径
