...
- **衍生发现（战役级红旗，比 H-R7c 本身重要）：基线引擎隐性「最短作业优先」调度保费**。build_equity_curve 的同日候选排序键 = exit_date（占用日相同时最早出场先进场）——下单时不可知的未来信息，在容量约束下（96% 候选被弃）等价于**先知调度器**。多序实测（创板池，冻结 PM；首版「symbol 序」标签有误——priority 注入实际排出降序，已更正）：
  | 调度序 | 外层2026 ann | taken | 全期 ann |
  |---|---|---|---|
  | exit_date 序（引擎基线=先知） | +402.0% | 445 | +58.1% |
  | symbol 升序 | −4.7% | 54 | +3.7% |
  | symbol 降序（首版误标为「symbol 序」） | +20.0% | 61 | +10.4% |
  | 随机序 21 种子中位 | **+5.7%**（极差 −12.9~+35.2%） | 60 | **+4.9%** |
  可部署逐年中位：+8.2/−5.9/+0.1/+5.2/+23.9/+5.7%（2021-26）——**调度保费逐年段一致非 2026 特化；且可部署序族本身方差巨大（单序不可作标准）→ 正式可部署口径=多种子中位数**。
- **双口径落地（用户裁决 A，`portfolio_metrics_dual` + `logs/quality/dual_caliber_baseline.json`）**：oracle（exit_date 序，历史连续性锚，仅内部）/ deployable（random 多种子中位，对外声明标准）。**R7a 倾斜在可部署序下（裁决点 B 的第一份数据）：外层 Δ中位 +0.2pp（极差 −12.4~+7.7 跨零）、全期 Δ中位 +0.2pp——+26.9pp 确认值基本是先知口径 artifact，H-R7a 的部署价值存疑**（除非在可部署口径下重证）。
- **含义分层（重要）**：①**同背比较不受染**——历史上所有 A/B（R6-11b 结构对照/R6-11c 冻结 −242pp/H-R7a +26.9pp）两臂共享同一调度器，相对结论仍成立为假设；②**绝对读数受染**——+400% 对照线是先知口径；**正式可部署口径（双口径裁决落地）：外层中位 +5.7%（21 种子极差 −12.9~+35.2）、全期 +4.9%、逐年中位 +8.2/−5.9/+0.1/+5.2/+23.9/+5.7%**；③**双轨实测天然检验此发现**：实盘（计划序挂单）若远低于 +400% 量级即为本机制的实锤——这是可证伪预测；④排序键自 P0-1（2026-08-02）即存在，为确定性选择从未被当作容量分配政策审查（T+1/幽灵成交同族的保真度盲区，红旗②排队家族的第四面）。
- **裁决状态（2026-08-26 深夜）**：A=**双口径并报（已落地）**——`PositionModel.queue_order` 三档（exit_date/symbol/random+seed/priority）+ `portfolio_metrics_dual`（deployable=21 种子随机序逐指标中位）+ `logs/quality/dual_caliber_baseline.json` 冠军双口径快照（换代重跑即刷新）；B. **H-R7a 在可部署序下第一份数据已出**：外层 Δ中位 +0.2pp（极差 −12.4~+7.7 跨零）、全期 Δ中位 +0.2pp——**+26.9pp 确认值基本是先知口径 artifact，部署价值存疑**（正式重证须预登记受控）；C. **H-R7d 登记（下假设，优先级因此升高）**：吞吐感知可部署优先级——用 at_signal 特征预测快速了结（tp2 快速兑现 vs 慢拖）优先进场，可部署地回收调度保费（oracle 445 vs 可部署 ~60 taken 的吞吐差是最大杠杆面）；特征须经 P1 同款硬闸（对 holding_bars 的判别力普查）+ 预登记受控，非本轮展开。
