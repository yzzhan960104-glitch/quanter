...
【Step4b 迁出】离场纯函数 check_exit + ExitAction/ExitReason/ExitDecision 已抽至
    ``caisen/engines/exit_logic.py``（纯逻辑归 engines），本模块经 re-export 引用，
    旧路径 ``from caisen.infra.execution import check_exit`` 零改动同源（strangler 铁律①）。
    backtest_replay._simulate_one_trade 改调 check_exit 离场（消除双源真理，见模块顶
    "离场纯函数 check_exit"段）。check_exit 逻辑【零改动】只搬位置。
...
物理定位（CLAUDE.md 极简 + 显式 + 无黑盒原则）：
    ...
    Step4b 起：实盘 ExecutionEngine（本模块 tick_exit）与回放验证器 backtest_replay
    （_simulate_one_trade）共用 caisen.engines.exit_logic.check_exit，杜绝"回放一套
    离场规则 / 实盘另一套"的双源真理隐患。
