# -*- coding: utf-8 -*-
"""数据层核心配置（数据湖/Tushare/宏观另类数据源）—— 从 config.py 拆出（归属：数据层）。
...
退役说明（2026-07-27）：
    原 JQDATA_CONFIG（JQData 分钟级配额闸门）与 AKSHARE_CONFIG（活跃池/板块阈值）随
    AKShare/JQData 非主源数据整体退役而删除——连同 a_shares_1min / jq_shards /
    a_shares_active / sector / sector_momentum / north_flow / dragon_list 7 项数据、
    JQDataClient + sync_jqdata_1min/sync_sector_daily/sync_north_flow/sync_dragon_list 4 脚本、
    LakeDataFetcher.dynamic_top50 活跃池路由一并下线。AKShareClient 主体保留（macro_credit
    仍用 fetch_macro_raw/fetch_daily_hist）。北向资金权威源 = moneyflow_hsgt.north_money。
"""
import os as _os
...
# 设计意图：DataLakeReader 通过此 dict 按 key 寻址并缓存已打开的 Parquet 句柄，
...
LAKE_CONFIG["lakes"] = {
    ...
    # Plan A Task 10：股东/解禁/停牌（四湖独立新增，不复用 sector）
