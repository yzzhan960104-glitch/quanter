...
（待迁·Step4 移出 caisen 包至横切 viz 层）本模块当前物理位于 caisen/infra/ 过渡子包，
Step4 将连同 storage/execution/replay_* 整体迁出 caisen 包。viz_* 属横切可视化层
（非策略本体、非执行编排），当前位置仅为 Step3 分层重构的中间态。
