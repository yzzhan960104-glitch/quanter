...
class StrategyConfig(BaseModel):
    ...
    min_rr_ratio: float = Field(
        1.5,
        description=(
            "盈亏比下限(回踩均价入场+第n波目标公式，Phase2 Bug4 修后)。"
            "【定标 2026-07-11】默认 1.5 为保守值——replay 重复计数 bug 已修(commit 501353f，统计可信)，"
            "但去重后近3年 sample=50 的 W 底 exit avg_rr=-0.65(近年表现差) + 全市场 replay 单机太慢"
            "(O(标的×T²))拿不到大样本参数扫描，未能数据驱动定标。1.5 保守：标准 W 底 entry rr≈1.4 "
            "暂被拦(待策略/参数审视近年 W 底为何亏损，或性能优化后全市场扫描定标)。旧默认 3.0 全拦。"
        ),
    )
