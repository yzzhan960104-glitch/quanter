...
Step3.4 + Step4 物理迁移整体完成后，本子包下已无实体逻辑——所有模块均已迁出：
  ...
  - 批 C（viz_static + viz_interactive）：Step4f 曾迁入横切 viz/ 顶层包
    （viz.viz_static / viz.viz_interactive）。Task 1.4：caisen 形态可视化整体退役，
    viz.viz_static/viz.viz_interactive 真身 + infra/viz_*.py 垫片全删（pattern 可视化
    是 caisen 形态专属 dead code，无 live importer）。
...
# 批 C re-export 已删（Task 1.4：viz_static/viz_interactive 真身 viz.viz_* + 垫片全删）
