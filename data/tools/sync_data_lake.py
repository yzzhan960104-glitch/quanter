...
def build_multiindex(shard_dir: str, out: str) -> None:
    ...
    # write-side 守卫 + 原子落盘（T13-A deferred + G5 原子写）：safe_overwrite 内部完成
    # 「守卫 + tmp + fsync + os.replace」原子写入，调用方不再紧跟 to_parquet（防半截损坏）。
    # build_multiindex 是全量重算合并，正常行数稳定；骤降（new < 现有×0.9）= 疑残片/部分回采
    # → 拒写保护历史。
...
def main(years: int, out: str, resume: bool = True, limit: int | None = None) -> None:
    ...
    for ts_code in tqdm(codes):
        ...
        # write-side 守卫 + 原子落盘（T13-A deferred + G5 原子写）：safe_overwrite 内部完成
        # 「守卫 + tmp + fsync + os.replace」原子写入，调用方不再紧跟 to_parquet（防半截损坏）。
        # shard 首次写无基线→放行；续传覆盖时若新 shard 残片（行数骤降 new < 现有×0.9）→
        # 拒写防部分回采抹历史。
        from data.integrity import safe_overwrite
